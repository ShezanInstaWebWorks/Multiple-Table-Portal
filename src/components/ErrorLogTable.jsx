...
export default function ErrorLogTable({ data }) {
  
  const error_data = data?.errorLogs;
  
  console.log("jjjjj",error_data);
...
const compareID = (a, b, order) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = error_data.map((n) => n.id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - error_data.length) : 0;
  ...
  const visibleRows = React.useMemo(
    ...
    [error_data,order, orderBy, page, rowsPerPage]
  );
  ...
  return (
    <Box sx={{ width: "80%", padding: "50px 50px 50px 150px" }}>
      <h3 style={{ textAlign: "left" }}>Error Log</h3>
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell 
                    onClick={() => handleCopyToClipboard(JSON.stringify(row?.details))}
                    sx={{ paddingLeft: "20px" }} align="left">
                      {truncateString(JSON.stringify(row?.details),30)}
                    </TableCell>
                    <TableCell 
                    onClick={() => handleCopyToClipboard(row?.function_ref)}
                    sx={{ paddingLeft: "20px" }} align="left">
                      {truncateString(row?.function_ref,30)}
                    </TableCell>
                    <TableCell sx={{ paddingLeft: "20px" }} align="left">
                      {row?.org_id}
                    </TableCell>
